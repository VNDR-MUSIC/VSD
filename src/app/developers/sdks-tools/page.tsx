...
const SdkCard = ({ icon: Icon, title, status, children, githubLink }: { icon: React.ElementType, title: string, status: string, children: React.ReactNode, githubLink?: string }) => (
  <Card className="shadow-lg bg-card/80 backdrop-blur-sm h-full"> {/* Added h-full */}
    <CardHeader>
  ...
  return (
    <div className="space-y-12 sm:space-y-16 py-8"> {/* Adjusted spacing */}
      <header className="text-center">
        <Package className="h-12 w-12 sm:h-16 sm:w-16 text-primary mx-auto mb-4" /> {/* Adjusted icon size */}
        <h1 className="font-headline text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-primary">SDKs & Tools</h1> {/* Adjusted font size */}
      ...
      <SdkCard icon={Layers} title="JavaScript/TypeScript SDK" status="Alpha">
        ...
        <h4 className="text-lg sm:text-xl mt-4 mb-2">Key Features:</h4> {/* Adjusted font size */}
        <ul>
...
getVsdBalance('0xYourAccountAddress', 'YOUR_ETHEREUM_RPC_URL');`}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8"> {/* Adjusted gap */}
        <SdkCard icon={RadioTower} title="Python SDK" status="Planned">
          ...
          <p className="mt-4 text-xs sm:text-sm">Stay tuned for updates on its development progress.</p> {/* Adjusted font size */}
        </SdkCard>
        ...
        <SdkCard icon={RadioTower} title="Go SDK" status="Planned">
          ...
          <p><strong>Expected Features:</strong> Core protocol interactions, type-safe smart contract bindings, transaction management.</p>
           <p className="mt-4 text-xs sm:text-sm">Follow our announcements for availability.</p> {/* Adjusted font size */}
        </SdkCard>
        ...
        <SdkCard icon={TerminalSquare} title="CLI Tools" status="Planned">
          ...
          <p className="mt-4 text-xs sm:text-sm">Development will commence after core SDKs are stable.</p> {/* Adjusted font size */}
        </SdkCard>
      </div>
      ...
      <section className="text-center py-8">
        <h2 className="font-headline text-2xl sm:text-3xl font-bold mb-4">Stay Updated & Contribute</h2> {/* Adjusted font size */}
        <p className="text-muted-foreground max-w-xl mx-auto mb-6 text-sm sm:text-base"> {/* Adjusted font size */}
